...
def compute_cm(classifier, test_data):
    ...
    Returns:
        Confusion matrix
    """
    confusion_matrix = ConfusionMatrix(classifier.label_2wdict, classifier.label_2wdict)
...
class ConfusionMatrix(object):
    ...
    def print_out(self):
        ...
        header = [' '] + [self.index2label[i] for i in range(self.num_classes)]
        rows = []
        # putting labels to the first column of rhw matrix
        for i in range(self.num_classes):
            row = [self.index2label[i]] + [str(self.matrix[i, j]) for j in range(len(self.matrix[i,]))]
            rows.append(row)
        print('\n\nConfusion Matrix')
        print('--------------')
